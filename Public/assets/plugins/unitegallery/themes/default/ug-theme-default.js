if (typeof g_ugFunctions != "undefined") g_ugFunctions.registerTheme("default");
else
...
function UGTheme_default() {
  ...
  function modifyOptions() {
    ...
    g_options.slider_textpanel_css_title["text-align"] =
      g_options.theme_text_align;
    g_options.slider_textpanel_css_description["text-align"] =
      g_options.theme_text_align;
  ...
  function setHtml() {
    ...
    if (g_options.theme_enable_hidepanel_button == true)
      g_objButtonHidePanel = g_objPanel.children(
  ...
  function initPanel() {
    ...
    if (g_objTextPanel) {
      ...
      if (g_objButtonHidePanel)
        maxButtonsHeight = Math.max(
          g_objButtonHidePanel.outerHeight(),
          maxButtonsHeight
        );

      if (g_objButtonFullscreen)
        maxButtonsHeight = Math.max(
          g_objButtonFullscreen.outerHeight(),
          maxButtonsHeight
        );

      if (g_objButtonPlay)
        maxButtonsHeight = Math.max(
          g_objButtonPlay.outerHeight(),
          maxButtonsHeight
        );
    ...
    if (g_objButtonHidePanel) {
      var buttonTip = g_objButtonHidePanel.children(
      ...
      if (g_objTextPanel) {
        var objHideButtonBG = g_objButtonHidePanel.children(
    ...
    if (g_objTextPanel) {
      ...
      textPanelOptions.slider_textpanel_padding_right =
        g_options.theme_text_padding_right + paddingPlayButton;
      textPanelOptions.slider_textpanel_padding_left =
        g_options.theme_text_padding_left;
    ...
    if (g_objButtonHidePanel) {
      if (g_objTextPanel)
        //place at the beginning of hte panel
  ...
  function checkHidePanel() {
    ...
    if (needToHide == true) hidePanel(true);
    else showPanel(true);
  }
  ...
  function hidePanel(noAnimation) {
    ...
    if (noAnimation == true)
      g_functions.placeElement(g_objPanel, 0, newPanelPosY);
    else placePanelAnimation(newPanelPosY, placeSlider);
  ...
  function onHidePanelClick(event) {
    ...
    if (isPanelHidden() == true) showPanel();
    else hidePanel();
  }
