...
$(function () {
  $("#slide-submenu").on("click", function () {
    $("#filters-block").fadeOut("slide", function () {
      ...
      $("#hotel-listing-block")
        .removeClass("col-sm-9 col-md-9")
        .addClass("col-sm-11 col-md-11");
  ...
  $(".mini-submenu-container").on("click", function () {
    ...
    $("#hotel-listing-block")
      .removeClass("col-sm-11 col-md-11")
      .addClass("col-sm-9 col-md-9");
    $(".swap-tiles li.active").click();
  });
});
...
function getFilterData() {
  ...
  //### Sorting
  data.orderBy = $("#sort-box").find(":checked").data("order");
  data.orderType = $("#sort-box").find(":checked").val();
...
function renderListTemplate(response) {
  if (
    ...
    typeof response.result.hotels !== "undefined" &&
    response.result.hotels.length > 0
  ) {
    var source = $("#hotel-list-block-tmpl").html();
    var template = Handlebars.compile(source);
    var htmlOutput = template({
      records: response.result.hotels,
  ...
  if (
    ...
    var myOptions = {
      center: new google.maps.LatLng(
        HotelsList[0].HotelInfo.Latitude,
        HotelsList[0].HotelInfo.Longitude
      ),
      zoom: 13,
    ...
    hotelsListingMap = new google.maps.Map(
    ...
    HotelsList.forEach(function (hotel) {
      if (hotel.HotelInfo.Latitude && hotel.HotelInfo.Longitude) {
        centerMap = new google.maps.LatLng(
          hotel.HotelInfo.Latitude,
          hotel.HotelInfo.Longitude
        );
        var marker = new MarkerWithLabel({
          position: centerMap,
          map: hotelsListingMap,
          labelContent:
            hotel.HotelPrice.Currency + " " + hotel.HotelPrice.MinPrice,
